      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
          {cards.map((card, index) => (
            ...
            >
              <div
                className={`h-48 md:h-60 relative overflow-hidden ${card.blogStyle ? "bg-brand-900/30 flex items-center justify-center" : "bg-brand-950 flex items-center justify-center"}`}
              >
                {card.blogStyle ? (
                  <>
                  ...
                  <>
                    ...
                    />
                    {card.videoStyle && (
                      <div className="w-12 h-12 md:w-16 md:h-16 bg-brand-900/80 backdrop-blur-md rounded-full flex items-center justify-center border border-gold-500/30 z-10 group-hover:bg-gold-500 group-hover:text-brand-950 transition-all">
                        {React.cloneElement(card.icon as React.ReactElement, {
                          className: "w-5 h-5 md:w-6 md:h-6 text-gold-400 fill-current group-hover:text-brand-950 ml-1",
              ...
              <div className="p-6 md:p-8 flex justify-between items-center">
                <div>
                  <h3 className="font-serif font-bold text-xl md:text-2xl text-brand-50 mb-1.5 md:mb-2">
                    {card.title}
                  </h3>
                  <p className="text-brand-100/70 font-light text-xs md:text-sm">
                    {card.subtitle}
                  </p>
                </div>
