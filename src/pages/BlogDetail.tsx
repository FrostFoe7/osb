...
const BlogDetail: React.FC<BlogDetailProps> = ({ onBack }) => {
    const location = useLocation();
    const blog = location.state?.blog as BlogItem;

    if (!blog) {
        return (
            <div className="pt-24 md:pt-32 pb-16 md:pb-24 min-h-screen flex items-center justify-center">
                <div className="text-center">
                    <p className="text-brand-100/60 mb-6">প্রবন্ধটি পাওয়া যায়নি।</p>
                    <Button onClick={onBack} variant="outline" className="border-gold-500/30 text-gold-400">
                        লিস্টে ফিরে যান
                    </Button>
                </div>
            </div>
